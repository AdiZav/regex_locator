...
"""
test_cli_tests.py runs tests that mimic user interaction with the script.
Each test simulates different user interactions and checks their outcome.
"""
...
def test_normal(capsys):
    ...
    main.main(**{'regex': "r'b\w{2}'", 'files': ["./tests/test_files/basic.txt"],
...
def test_missing_exp():
    ...
    with pytest.raises(Exception):
        main.main(**{'files': ["./tests/test_files/basic.txt"],
                              'color': "blue", 'machine': True})
        raised = True
...
def test_missing_file():
    ...
    raised = False
    with pytest.raises(Exception):
        main.main(**{'regex': "r'b\w{2}'", 'files': ["./tests/test_files/basic1.txt"],
                              'color': "blue", 'machine': True})
        raised = True
